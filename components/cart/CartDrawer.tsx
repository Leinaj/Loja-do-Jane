...
const FALLBACK_IMG = '/placeholder.png'; // coloque um png na pasta public
  ...
  return (
    <div className="space-y-4 p-4">
        ...
        {items.map((it) => {
          const imgSrc =
            typeof (it as any).image === 'string' && (it as any).image
              ? (it as any).image
              : FALLBACK_IMG;
          const altTxt = (it as any).name ?? 'Produto';

          return (
            <li key={String((it as any).id)} className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm opacity-70">
                  {formatBRL((it as any).price ?? 0)}
                </p>
              </div>

              <button
                onClick={() => remove((it as any).id)}
      ...
      <div className="flex gap-2 pt-2">
        <button onClick={clear} className="rounded-md border px-3 py-2">
          Limpar carrinho
        </button>
        <Link
          href="/checkout"
