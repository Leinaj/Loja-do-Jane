...
type Product = { id: string; name: string; price: number; oldPrice?: number; img: string; };
type Cart = Record<string, number>;

const WHATSAPP_RAW = '5544988606483';
...
const PRODUCTS: Product[] = [
  { id: 'camiseta-preta', name: 'Camiseta Preta', price: 69.9, oldPrice: 89.9, img: '/images/camiseta-preta.jpg' },
  { id: 'camiseta-branca', name: 'Camiseta Branca', price: 69.9, oldPrice: 89.9, img: '/images/camiseta-branca.jpg' },
...
export default function HomePage() {
  ...
  const removeOne = (id: string) =>
    setCart(c => { const q = (c[id] ?? 0) - 1; const n = { ...c }; if (q <= 0) delete n[id]; else n[id] = q; return n; });
  ...
  const finalizeWhatsApp = () => {
    ...
    const lines = [
      ...
      'Itens:',
      ...items.map(i => `• ${i.product.name} x${i.qty} — ${currency.format(i.line)}`),
      '', `Total: ${currency.format(total)}`, '',
      `Chave PIX: ${PIX_KEY}`, 'Forma de pagamento: Pix ou Cartão', '',
      'Podemos combinar entrega/retirada?'
    ];
    const waText = encodeURIComponent(lines.join('\n'));
    window.open(`https://wa.me/${WHATSAPP_RAW}?text=${waText}`, '_blank');
  };

  const copyPix = async () => {
    try { await navigator.clipboard.writeText(PIX_KEY); alert('Chave PIX copiada!'); }
    catch { alert('Copie manualmente: ' + PIX_KEY); }
  };
    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-200">
      <div className="text-xs sm:text-sm bg-neutral-900 text-neutral-400 border-b border-neutral-800">
        <div className="max-w-6xl mx-auto px-4 py-2 flex flex-wrap gap-x-6 gap-y-1 justify-between">
          <span>Bem-vinda à <b className="text-white">Loja da Jane</b> ✨</span>
          <div className="flex gap-4">
            <span>Suporte: WhatsApp <a className="text-emerald-400 underline" href={`https://wa.me/${WHATSAPP_RAW}`} target="_blank">{WHATSAPP_DISPLAY}</a></span>
            <span>Carrinho: {items.length} {items.length === 1 ? 'item' : 'itens'} — {currency.format(total)}</span>
          </div>
        </div>
      </div>
      ...
      <section id="home" className="max-w-6xl mx-auto px-4 pt-6">
        <div className="bg-neutral-900 rounded-2xl p-4 sm:p-6 border border-neutral-800">
          <div className="grid sm:grid-cols-2 gap-6 items-center">
            ...
            <div>
              <h1 className="text-3xl sm:text-4xl font-semibold">iPhone 6 <span className="text-emerald-400">Plus</span></h1>
              <p className="text-neutral-400 mt-2">Exemplo de banner. Para trocar, substitua o arquivo <code className="px-1 py-0.5 rounded bg-neutral-800 text-neutral-300">/public/banner.jpg</code>.</p>
              <div className="flex flex-wrap gap-3 mt-4">
                <a href="#catalogo" className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white">Ver produtos</a>
                <button onClick={finalizeWhatsApp} className="px-4 py-2 rounded-lg border border-neutral-700 hover:bg-neutral-800">Finalizar no WhatsApp</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div key={p.id} className="rounded-2xl overflow-hidden border border-neutral-800 bg-neutral-900">
              ...
              <div className="p-4">
                ...
                <div className="mt-1">
                  ...
                  {p.oldPrice && <span className="ml-2 text-neutral-400 line-through">{currency.format(p.oldPrice)}</span>}
                </div>
                <button onClick={() => addToCart(p.id)} className="w-full mt-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white py-2">Adicionar</button>
              </div>
            </div>
          ))}
        </div>
                ...
                {items.map(({ product, qty, line }) => (
                  <li key={product.id} className="py-3 flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-neutral-400 text-sm">{qty} x {currency.format(product.price)} = <b className="text-neutral-200">{currency.format(line)}</b></div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => removeOne(product.id)} className="px-3 py-1 rounded border border-neutral-700 hover:bg-neutral-800">−</button>
                      <button onClick={() => addToCart(product.id)} className="px-3 py-1 rounded border border-neutral-700 hover:bg-neutral-800">+</button>
                    </div>
                  </li>
                ))}
              </ul>
              <div className="mt-4 flex items-center justify-between">
                <div className="text-lg">Total: <b>{currency.format(total)}</b></div>
                <div className="flex gap-2">
                  <button onClick={clearCart} className="px-4 py-2 rounded border border-neutral-700 hover:bg-neutral-800">Limpar</button>
                  <button onClick={finalizeWhatsApp} className="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-500 text-white">Finalizar no WhatsApp</button>
                </div>
              </div>
            </>
          )}
      ...
      <section id="contato" className="max-w-6xl mx-auto px-4 mt-10">
        ...
        <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4">
          ...
          <a className="block text-emerald-400 underline" href={`https://wa.me/${WHATSAPP_RAW}`} target="_blank">{WHATSAPP_DISPLAY}</a>
          <div className="mt-4">
            <div className="text-neutral-300">Chave PIX</div>
            <div className="flex gap-2 mt-1">
              <input readOnly value={PIX_KEY} className="px-3 py-2 rounded-lg bg-neutral-
